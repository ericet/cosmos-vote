<template>
 ...
 <AccountsGrid :accounts="accounts" />
  ...
  <div class="mx-auto max-w-2xl h-72 mb-5 mt-4 block p-2.5 text-sm text-green-500 bg-black rounded-lg overflow-auto">
    <p v-for="(log, index) in logs" :key="index" class="font-normal" :class="{
...
export default {
    ...
    methods: {
        ...
        async voteProposalEvmos(wallet, chain, proposal, address) {
            ...
            const voteTx = createTxMsgVote(MAINNET_CHAIN, sender, {
            ...
            this.logit("ready", `【${address}】 is ready to vote '${this.getVoteOption(Number(proposal.vote))}' on proposal #${proposal.proposal_id}`);
            ...
            try {
                ...
                if (broadcastRes.tx_response.code === 0) {
                    this.logit("success", `【${address}】 voted '${this.getVoteOption(Number(proposal.vote))}'' proposal #${proposal.proposal_id}`);
                    proposal.status_code = "voted";
                }
                else {
                    this.logit("error", `【${address}】 failed to vote '${this.getVoteOption(Number(proposal.vote))}' on proposal #${proposal.proposal_id}: ${broadcastRes.tx_response.raw_log}`);
        ...
        async voteProposal(client, chain, proposal, address) {
            ...
            try {
                ...
                if (result.code == 0) {
                    ...
                    this.logit("error", `【${address}】 failed to vote '${this.getVoteOption(Number(proposal.vote))}' on proposal #${proposal.proposal_id}`);
                }
            }
            catch (err) {
                this.logit("error", `【${address}】 ${err}`);
        ...
        async getQueryClient(rpcEndpoint) {
            ...
            const queryClient = QueryClient.withExtensions(tendermint34Client, setupBankExtension, setupGovExtension);
            return queryClient;
        },
        async vote() {
            this.logs = [];
                    ...
                    for (let proposal of proposals) {
                        ...
                        try {
                            ...
                            if (chain.name === "Evmos") {
                                ...
                                client = await SigningStargateClient.connectWithSigner(chain.rpc, wallet);
                                accounts = await wallet.getAccounts();
                            }
                            const queryClient = await this.getQueryClient(chain.rpc);
                            for (let account of accounts) {
                                let voted = await this.hasVoted(queryClient, proposal, account.address);
                                if (!voted) {
                                    if (chain.name === "Evmos") {
                                        await this.voteProposalEvmos(wallet, chain, proposal, account.address);
                                    }
                                    else {
                                        await this.voteProposal(client, chain, proposal, account.address);
                                    }
                                }
                                else {
                                    this.logit("error", `【${account.address}】 has already voted proposal #${proposal.proposal_id}`);
                                    proposal.status_code = "voted";
                                }
                            }
                        }
